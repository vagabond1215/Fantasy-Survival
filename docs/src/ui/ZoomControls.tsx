import type { CameraState } from '../map/camera.ts';
...
export interface ZoomControlsOptions {
  ...
  styleButton?: (
    ...
    style?: { size?: number | string; fontSize?: string; variant?: 'chip' | 'square' | 'stacked' }
  ) => void;
}
...
export function createZoomControls(options: ZoomControlsOptions): ZoomControlsHandle {
  ...
  const zoomOutButton = createButton('−', 'Zoom out', { fontSize: '22px', variant: 'chip' });
  const zoomInButton = createButton('+', 'Zoom in', { fontSize: '22px', variant: 'chip' });
  ...
  const handle: ZoomControlsHandle = {
    ...
    update(state) {
      ...
      const baseline = Number.isFinite(state.baselineZoom) ? state.baselineZoom : 1;

      const clampedZoom = clamp(cameraZoom, 0, Number.isFinite(cameraMax) ? cameraMax : 8);
