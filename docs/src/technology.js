...
/**
 * @typedef {Object} TechnologyCost
 * @property {number|null} [research]
 * @property {Record<string, number>} [materials]
 */

/**
 * @typedef {Object} TechnologyEffectsUnlocks
...
/**
 * @typedef {Object} TechnologyEffects
 * @property {string} [description]
 * @property {TechnologyEffectsUnlocks} [unlocks]
 */
 ...
 * @property {string} [description]
 * @property {Array<string|number>} [prerequisites]
 * @property {TechnologyCost} [cost]
 * @property {TechnologyEffects} [effects]
 */

/**
 * @typedef {Object} NormalizedTechnologyDefinition
 * @property {string} id
 * @property {string} name
 ...
 * @property {{ research: number|null, materials: Record<string, number> }|null} cost
 * @property {{ description: string, unlocks: { technologies: string[], recipes: string[], buildings: string[], equipment: string[] } }} effects
 */
...
/** @type {Map<string, NormalizedTechnologyDefinition>} */
const registry = new Map();
let registryInitialized = false;

/**
 * @returns {Map<string, TechnologyState>}
 */
function ensureTechnologyMap() {
  const current = store.technologies;
  if (current instanceof Map) {
    return /** @type {Map<string, TechnologyState>} */ (current);
  }
  if (Array.isArray(current)) {
    store.technologies = new Map(current);
    return /** @type {Map<string, TechnologyState>} */ (store.technologies);
  }
...
/**
 * @param {TechnologyDefinition|NormalizedTechnologyDefinition|null|undefined} definition
 * @returns {NormalizedTechnologyDefinition|null}
 */
function normalizeDefinition(definition) {
  ...
  /** @type {NormalizedTechnologyDefinition} */
  const normalized = {
    id,
...
/**
 * @param {NormalizedTechnologyDefinition} definition
 * @param {Partial<TechnologyState> & Record<string, unknown>} [raw]
...
function ensureTechnologyState(id) {
  const normalizedId = String(id);
  const technologies = ensureTechnologyMap();
  if (!technologies.has(normalizedId)) {
    ...
    technologies.set(normalizedId, state);
  }
  return technologies.get(normalizedId);
}
