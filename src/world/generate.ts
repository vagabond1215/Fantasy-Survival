...
import type { CanonicalSeed } from './seed.ts';
import { canonicalizeSeed } from './seed.ts';
import { xorshift128plus } from './rng.ts';
...
const BiomeCodeFromId: Record<string, BiomeCode> = BiomeIds.reduce((acc, id, index) => {
  acc[id] = index as BiomeCode;
  return acc;
}, {} as Record<string, BiomeCode>);

export function biomeCodeToId(code: BiomeCode): BiomeId {
  return BiomeIds[code] ?? 'temperate-broadleaf';
}

export function biomeIdToCode(id: string): BiomeCode {
...
export type WorldArtifact = {
  ...
  readonly dimensions: Readonly<{ width: number; height: number; size: number }>;
...
type BiomeResourceCoefficients = {
  readonly woodMod: number;
  readonly openLand: number;
  readonly freshwaterPresence: number;
  readonly elevationVariance: number;
};

type BiomeSelection = {
  readonly code: BiomeCode;
  readonly score: number;
  readonly reason: number;
};

const biomeResourceCoefficients: readonly BiomeResourceCoefficients[] = BiomeIds.map(id => {
...
function sanitizeBias(value: number | undefined, maxMagnitude: number): number {
  if (!Number.isFinite(value)) return 0;
  const limited = Math.max(-maxMagnitude, Math.min(maxMagnitude, value));
  return Math.fround(limited);
}

function sanitizeConfig(params: WorldParams): GenerationConfig {
...
function fingerprintConfig(config: GenerationConfig): number {
  let hash = 0x811c9dc5;
...
function mix32(value: number, salt: number): number {
  let h = (value ^ salt) >>> 0;
  h ^= h >>> 16;
...
function normalized(index: number, max: number): number {
  if (max <= 1) return 0;
  return index / (max - 1);
}
...
function vegetationDensity(
  temperature: number,
  moisture: number,
  elevation: number,
  runoff: number,
  coeff: BiomeResourceCoefficients,
): number {
...
function orePotential(elevation: number, moisture: number, coeff: BiomeResourceCoefficients): number {
  return clamp01(0.35 + elevation * 0.45 + coeff.elevationVariance * 0.3 - moisture * 0.2);
}

function waterPotential(runoff: number, moisture: number, coeff: BiomeResourceCoefficients): number {
  return clamp01(runoff * 0.6 + moisture * 0.25 + coeff.freshwaterPresence * 0.4);
}

function fertilityPotential(
  moisture: number,
  temperature: number,
  elevation: number,
  vegetation: number,
...
function prepareSpawnSuggestions(scores: Float32Array, count: number): Uint32Array {
...
function selectBiome(
  ...
  function consider(code: BiomeCode, valid: boolean, score: number, reason: number): void {
    if (!valid) return;
  ...
  const fallbackTable: Record<TemperatureBandLabel, Partial<Record<MoistureBandLabel, BiomeCode>>> = {
    frigid: {
...
function makeBaseRng(seed: CanonicalSeed, config: GenerationConfig): ReturnType<typeof xorshift128plus> {
...
function generateWorldArtifact(seed: CanonicalSeed, config: GenerationConfig): WorldArtifact {
  const lanes = ensureSeedLanes(seed);
  ...
  for (let i = 0; i < size; i += 1) {
    ...
    const biomeData = Object.freeze({
      id: biomeCodeToId(biome[i] as BiomeCode),
...
export function generateWorld(params: WorldParams): Promise<WorldArtifact> {
  const config = sanitizeConfig(params);
  if (params.seed) {
    const artifact = generateWorldArtifact(params.seed, config);
    const hybrid = Object.assign(Promise.resolve(artifact) as Promise<WorldArtifact>, artifact);
    return hybrid;
  }

  const seedString = params.seedString ?? '';
  const pending = canonicalizeSeed(seedString).then(seed => generateWorldArtifact(seed, config));
  const hybrid = pending as Promise<WorldArtifact> & Partial<WorldArtifact>;
